...
namespace Battlehub.SplineEditor
{
    ...
    public class SplineBase : MonoBehaviour, IGL
    {
            ...
            if (m_points != null && m_points.Length > 0)
            {
                ...
                if (m_settings.Length != modesLength)
                {
                    ...
                    for (int i = oldLength; i < m_settings.Length; ++i)
                    {
                        ...
                        m_settings[i].Wrap = new Wrap(0.0f, 0.0f, 1.0f);
                    }
                }
            }
        }
        ...
        {
            Wrap current = m_settings[curveIndex].Wrap;
            Wrap next = m_settings[curveIndex + 1].Wrap;
        ...
        {
            return m_settings[(index + 1) / 3].Wrap;
        }
        ...
        {
            int settingIndex = (index + 1) / 3;
            m_settings[settingIndex].Wrap = wrap;
        ...
        protected bool RemoveCurve(int curveIndex, bool raise = true)
        {
            ...
            if (enforceMode)
            {
                EnforceMode(pointIndex + 1, raise);
            }
        ...
        private void InsertCurve(Vector3[] points, ControlPointSetting setting, ControlPointMode mode, int curveIndex, float length, bool enforceNeighbour, bool shrinkPreceding)
        {
            ...
            if (shrinkPreceding)
            {
                ...
                {
                    m_points[i] = shrinkedPoints[2 + i - pointIndex];
                }
            }
            ...
            if(shrinkPreceding)
            {
                ...
                {
                    m_points[i + 1] = points[i - pointIndex];
                }
            }
            else
            {
        ...
        {
            InsertCurve(points, GetSetting(curveIndex * 3), GetControlPointMode(curveIndex * 3), curveIndex, length, enforceNeighbour, shrinkPreceding);
        }
